...
-- 5. Ensure prescription_requests policies are properly restrictive
-- (The existing policies look good, but let's verify they're working correctly)
-- Customers can only see their own requests, doctors can see assigned or unassigned requests

-- 6. Ensure prescriptions policies are working correctly
-- (The existing policies look good - customers see via request ownership, doctors see their own)

-- 7. Ensure appointments policies are working correctly  
-- (The existing policies look good - customers see their own, doctors see their assigned)

-- 8. Fix storage policies for private buckets
-- Ensure prescriptions bucket is properly secured
CREATE POLICY IF NOT EXISTS "Users can view their own prescriptions in storage"
ON storage.objects
FOR SELECT
USING (
...
CREATE POLICY IF NOT EXISTS "Users can view their own private prescriptions in storage"
ON storage.objects  
FOR SELECT
USING (
...
CREATE POLICY IF NOT EXISTS "Users can view their own consultation recordings"
ON storage.objects
FOR SELECT  
USING (
