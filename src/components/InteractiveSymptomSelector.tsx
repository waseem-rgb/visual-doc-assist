    ...
    try {
      ...
      const { data: doctorSmsResult, error: doctorSmsError } = await supabase.functions.invoke('send-sms-notification', {
        body: {
          ...
          patientName: patientData.name,
          message: `New instant consultation from ${patientData.name} (${patientData.age}${patientData.gender?.charAt(0)?.toUpperCase() || ''}). Please login to review: https://9ef9f016-bd2c-406c-a384-b239fe6ec4e2.lovable.dev/doctor/login`
        }
      });
