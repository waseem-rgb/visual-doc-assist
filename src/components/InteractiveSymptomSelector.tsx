    ...
    try {
      ...
      const { data: doctorSmsResult, error: doctorSmsError } = await supabase.functions.invoke('send-sms-notification', {
        body: {
          ...
          patientName: patientData.name,
          message: `New instant consultation from ${patientData.name} (${patientData.age}${patientData.gender?.charAt(0)?.toUpperCase() || ''}). Please check your VrDoc portal to review.`
        }
      });
