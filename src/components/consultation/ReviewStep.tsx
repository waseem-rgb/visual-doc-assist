        ...
        try {
          ...
          const { data: doctorSmsResult, error: doctorSmsError } = await supabase.functions.invoke('send-sms-notification', {
            body: {
              ...
              patientName: patientData.name,
              message: `New prescription request from ${patientData.name} (${patientData.age}${patientData.gender?.charAt(0)?.toUpperCase() || ''}). Symptoms: ${selectedSymptoms.join(', ')}. Please check your VrDoc portal to review.`
            }
          });
