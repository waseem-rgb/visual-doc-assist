...
const GeneralSymptoms = ({ patientData }: GeneralSymptomsProps) => {
  ...
  const handleClinicalSubmit = async () => {
    try {
      ...
      try {
        console.log('📱 [DOCTOR SMS] Sending notification to doctor for new general symptoms case');
        const { data: doctorSmsResult, error: doctorSmsError } = await supabase.functions.invoke('send-sms-notification', {
          body: {
            ...
            patientName: patientData.name,
            message: `New general symptoms consultation from ${patientData.name} (${patientData.age}${patientData.gender?.charAt(0)?.toUpperCase() || ''}). Please check your VrDoc portal to review.`
          }
        });
